...
export const creatorSignalAccessibilityContracts: Readonly<Record<string, ComponentLibraryAccessibilityContract>> = {
  ...
  'creator-signal.site.crm-iframe-form': contract([
    ...
    check({ rule: 'a11y.provider-fallback', category: 'provider', enforcement: 'automated', severity: 'error', fields: ['formUrl', 'iframeTitle', 'fallbackLabel'], summary: 'Embedded CRM Form requires an approved HTTPS form URL, accessible iframe title and fallback link.', remediation: 'Use the approved CRM URL, give the iframe a concise title and retain a link that opens the same form outside the page.' }),
    manual('a11y.no-javascript-fallback', 'no-javascript', 'The fallback link must remain available when the resize runtime cannot run.', 'Verify the published link opens the CRM form in a new tab without JavaScript.'),
    contrast(), touch(),
  ]),
