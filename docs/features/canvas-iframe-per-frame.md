...
- Wheel, pointer, and keyboard events are forwarded from inside the iframe to the parent's gesture / reorder-drag / shortcut handlers. Native `Tab` navigation remains active so editor-owned canvas node roots participate in the sequential keyboard order; nested interactive HTML emitted by a plugin preview is removed from that editor-only order without changing published output.
