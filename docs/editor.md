...
- `ExplorerPanel` — the consolidated navigation panel. One `<Panel>` shell hosts a top `SegmentedControl` with **Layers / Site / Code / Media** tabs (default **Layers**). While Layers is active, a second **Components / HTML** control selects the projection rendered by `src/admin/pages/site/panels/LayersPanel/LayersPanel.tsx`:
  - **Components** → `ComponentLayersTree`, an author-facing hierarchy projected by the pure `componentTreeProjection.ts` function from the same active `NodeTree`. Only instances inserted from the Component Library carry the required catalogue identity and appear here, including catalogue-stamped Visual Component references and their governed slots. Ordinary imported/freeform nodes, raw Visual Component references and their slots stay in HTML view; nested catalogue instances are hoisted through those wrappers in page order. A page with no catalogue instances shows an explicit empty state. Catalogue pattern implementation nodes collapse behind their boundary, and `layerSelection.ts` maps a canvas selection inside a governed boundary to its nearest visible owner while restoring the exact HTML selection when the user switches back. The add action opens the governed Component Library, while Properties switches to `ComponentPropertiesView` and exposes only registered fields, approved presets/variants, lifecycle state, slots and guidance.
...
- **Components:** explicit `ComponentLibraryEntry` registrations only. Entries
  reference primitive modules, Visual Component implementations or patterns;
  raw `site.visualComponents` definitions are not inferred into this governed
  surface. Descriptions and catalogue result badges identify the owning
  provider, and the full library supports provider-level filtering.
