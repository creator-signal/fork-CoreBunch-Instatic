...
Component Library:
- site_list_component_library returns the editor's live governed catalogue, including plugin-owned entries, with entry/version ids, fields, preset/variant ids, slots, constraints, dependencies, and accessibility contracts. Call site_check_accessibility after an authoring change when accessibility diagnostics are relevant; it returns deterministic instance-and-field results and marks only site-policy blockers as publication-blocking.
