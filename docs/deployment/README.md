...
Instatic is one Bun server packaged by the root `Dockerfile`. The server reads runtime configuration from `server/config.ts`: `PORT`, `DATABASE_URL`/`DATABASE_URL_FILE`, `UPLOADS_DIR`, `STATIC_DIR`, `PUBLIC_ORIGIN`, `TRUSTED_PROXY_CIDRS`, optional `MINIO_*` values, browser-only `INSTATIC_PUBLIC_CONNECT_ORIGINS`, and optional Admin/server GlitchTip settings. Reversible server secrets, including AI provider credentials, plugin secret settings, and MFA TOTP seeds, are encrypted with `INSTATIC_SECRET_KEY` or `INSTATIC_SECRET_KEY_FILE` when configured. Database migrations run automatically on boot in `server/index.ts`.
